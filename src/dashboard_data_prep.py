...
* The website currently only shows 6 of our 8 aspects (Food, Comfort,
  Cleanliness, Staff, Location, Facilities — missing Value, Noise). This
...
# All 8 aspects our NLP pipeline scores. The website today only surfaces 6
...
WEBSITE_ASPECTS = ["food", "comfort", "cleanliness", "staff", "location", "facilities"]
    ...
    categories = [
        {
            ...
            "icon": {"food": "utensils", "comfort": "bed", "cleanliness": "sparkles",
                      "staff": "users", "location": "map-pin", "facilities": "wrench"}[a],
...
def load_hotel_report(path: Path) -> dict:
    """Teammate's separate Ridge-regression model output (score_goals,
    action_plan, aspect_details per hotel), keyed by hotel_slug so it can be
    joined against our own data using the same slugify() convention as
    api/db.py. Not generated by this pipeline — see data/website_hotel_report.json
    (gitignored; copy it there before running with a hotel that has report data).
    """
    with open(path, encoding="utf-8") as f:
        records = json.load(f)
    return {slugify(r["hotel_name"], separator="_"): r for r in records}
...
def main() -> None:
    ...
    dropped = ", ".join(title(a) for a in ASPECTS if a not in WEBSITE_ASPECTS)
    print(f"\n=== 7. Aspects the website currently drops ({dropped}) ===")
    ...
    hotel_slug = slugify(focus_hotel, separator="_")

    report_path = Path(os.environ.get("HOTEL_REPORT_PATH", PROJECT_ROOT / "data" / "website_hotel_report.json"))
    if report_path.exists():
        report = load_hotel_report(report_path).get(hotel_slug)
        if report:
            website_data["SCORE_GOALS"] = report["score_goals"]
            website_data["ACTION_PLAN"] = report["action_plan"]
            website_data["ASPECT_DETAILS"] = report["aspect_details"]
            print(f"Merged score-improvement report data for '{focus_hotel}'.")
        else:
            print(f"No score-improvement report entry for '{focus_hotel}' (slug={hotel_slug}) — skipping merge.")
    else:
        print(f"No score-improvement report found at {report_path} — skipping merge.")
